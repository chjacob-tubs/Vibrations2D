...
class timedomain(Calc2dir_base):
    ...
    def __init__(self, freqs: np.ndarray, dipoles: np.ndarray, **params):
        ...
        if 'n_t' in params:
            ...
            if self.print_output:
                print('Set the number of time points '
                      '(n_t) to', str(self.n_t)+'.')
        else:
            self.n_t = 128
            if self.print_output:
        ...
        if 'dt' in params:
            ...
            if self.print_output:
                print('Set the time step length (dt) '
                      'to', self.dt, 'ps.')
        else:
            self.dt = 0.25
            if self.print_output:
        ...
        if 't2' in params:
            self.t2 = params.get('t2')
            if self.print_output:
                print('Set the population time (t2) '
                      'to', self.t2, 'ps.')
        else:
            self.t2 = 0
            if self.print_output:
        ...
        if 'omega_off' in params:
            ...
            if self.print_output:
                print('Set the omega offset value '
    ...
    def calc_fourpointcorr(self, pathway: str,
                           fak1: float, fak2: float, fak3: float,
                           *mus) -> float:
        '''
    ...
    def calc_diagrams_slow(self) -> np.ndarray:
        ...
        R_1, R_2 and R_3 are rephasing diagrams
        and R_4, R_5 and R_6 are non-rephasing diagrams.
    ...
    def calc_diagrams(self):
        ...
        mu = self.dipoles[0][1:self.noscill+1]
        mu2 = self._get_secexc_dipoles()
    ...
    def calc_sum_diagram(self,
                         R_a: np.ndarray,
                         R_b: np.ndarray,
                         R_c: np.ndarray) -> np.ndarray:
        '''
