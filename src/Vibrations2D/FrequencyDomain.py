...
class frequencydomain(Calc2dir_base):
    ...
    def __init__(self, freqs, dipoles, **params):
        ...
        if 'print_output' in params:
            ...
            if self.print_output:
                print('Prints all output. '
                      'To suppress printed output use '
                      'print_output=False.')
        else:
            self.print_output = True
            if self.print_output:
                print('Prints all output (default). '
                      'To suppress printed output use '
        ...
        if 'n_grid' in params:
            ...
            if self.print_output:
                print('Set the number of grid points '
        ...
        if 'T2' in params:
            ...
            if self.print_output:
                print('Set the dephasing time (T2) '
                      'to', self.T2, 'ps.')
        else:
            self.T2 = 2
            if self.print_output:
        ...
        if 't2' in params:
            self.t2 = params.get('t2')
            if self.print_output:
                print('Set the population time (t2) to',
                      self.t2, 'ps.')
        else:
            self.t2 = 0
            if self.print_output:
        ...
        if 'pol' in params:
            ...
            if self.print_output:
                print('Set the polarization (pol) '
        ...
        if 'pol_list' in params:
            ...
            if self.print_output:
                print('Set the polarization angles '
    ...
    def calc_excitation(self) -> list:
        ...
        for i in range(1, self.noscill+1):
            for j in range(self.noscill, len(intmat)):
                ...
                if self.print_output:
                    print('Excitation from energy level', i,
                          'to', j, 'at (',
                          np.around(x_coor, 2),
                          ',', np.around(y_coor, 2),
                          ') rcm and '
                          'intensity: ',
                          np.around(exc_inten, 2))
        ...
        for i in range(self.noscill+1):
            for j in range(len(intmat)):
                if j == 0 and i > j:
                    ...
                    if self.print_output:
                        print('Stim. emission from energy '
                              'level', i, 'to', j, 'at (',
                              np.around(x_coor, 2), ',',
                              np.around(y_coor, 2),
                              ') rcm and '
    ...
    def _get_2d_spectrum(self, xmin: float, xmax: float,
                         steps=2000, gamma=5, ftype='lorentz') -> np.ndarray:
        '''
    ...
    def calculate_S(self, axis):
        ...
        # get the fundamental transition dipoles mu
        # and the higher excited states mu2
        ...
        vfourpoint = np.vectorize(lambda i, j, k, l:
                                  self.calc_fourpointcorr_mat(fak1, fak2,
                                                              fak3,
                                                              mu[i], mu[j],
                                                              mu[k], mu[l]),
        ...
        for i in range(self.noscill):
            for j in range(self.noscill):
                ...
                for k in range(n_exc_oscill):
                    ...
                    S3 += (f_ji_kji[i][j][k]
                           * S3_func(-_w.T, _w, omega[j],
                                     (omega2[k]-omega[j]))
                           * t2_param(omega[i], omega[j])
                           )
                    S6 += (f_ji_kij[i][j][k]
                           * S6_func(_w.T, _w, omega[j],
                                     (omega2[k]-omega[i]))
